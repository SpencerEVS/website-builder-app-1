/*
 * WindowPanel Component Archive
 * Date: 2025-12-18
 * Version: Overlay Menu Bar + Transparent Background for JS Execution
 * 
 * Features implemented in this version:
 * 1. Menu bar overlays on top of content instead of shifting it down
 * 2. Transparent background for JavaScript windows in execution mode
 * 3. Enhanced menu bar styling with backdrop blur and shadow
 * 
 * Changes made:
 * - Removed dynamic padding from window-content (no longer shifts down)
 * - Added conditional transparent background for JS execution mode
 * - Enhanced header styling with box shadow for better overlay visibility
 * - Menu bar now truly overlays without affecting content positioning
 */
    ...
    const executeJavaScript = () => {
        ...
        setTimeout(() => {
            try {
                ...
                // Prepare API variables for sandbox
                const apiVariables: Record<string, any> = {};
                dataConnections.apiConnections.forEach(connection => {
                    connection.variables.forEach(variable => {
                        const key = `${connection.name}_${variable.name}`;
                        // For now, we'll use placeholder values since the actual API response parsing isn't implemented yet
                        apiVariables[key] = `${variable.type}_value_${variable.name}`;
                    });
                });
    ...
    return (
        <div 
            ...
            style={{
                ...
                backgroundColor: window.isOffCanvas ? '#ffe6e6' : (window.type === 'javascript' && hasExecuted && !showCodeEditor ? 'transparent' : 'white'),
                borderRadius: '4px',
                    ...
                    {window.type === 'javascript' ? (
                        ...
                        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div 
                                style={{
                                    border: '1px solid #ddd',
                                    padding: '8px',
                                    height: (isHovered || isSelected) ? '35%' : '100%',
                                    overflow: 'auto',
                        ...
                        <div 
                            style={{ 
                                ...
                                flexDirection: 'column',
                                border: window.content ? '2px solid #6f42c1' : '2px dashed #6f42c1',
                                borderRadius: '8px',
                                padding: '20px',
